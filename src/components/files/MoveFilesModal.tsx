...
export function MoveFilesModal({ 
  ...
  const handleMove = async () => {
    ...
    try {
      ...
      const updatePromises = selectedFiles.map(async (file) => {
        ...
        if (error) {
          throw new Error(`Failed to move file ${file.displayName}: ${error.message}`);
        }
      ...
      toast({
        title: "Success",
        description: `Moved ${selectedFiles.length} file(s) to "${targetFolder}" folder`,
      });
