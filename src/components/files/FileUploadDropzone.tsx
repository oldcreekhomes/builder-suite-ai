...
export function FileUploadDropzone({
  ...
  const [queuedFolders, setQueuedFolders] = useState<Array<{
    id: string;
    rootName: string;
    files: File[];
    totalFiles: number;
  ...
  const processFiles = async (files: File[]) => {
      ...
      for (const folderPath of folderPaths) {
        try {
          ...
          const {
            data: uploadData,
            error: uploadError
          } = await supabase.storage.from('project-files').upload(keeperFileName, emptyFile);
          if (!uploadError) {
            await supabase.from('project_files').insert({
              project_id: projectId,
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      const progressInterval = setInterval(() => {
        setUploadingFiles(prev => prev.map((upload, index) => upload.file === file ? {
          ...upload,
      ...
      setUploadingFiles(prev => prev.map(upload => upload.file === file ? {
        ...upload,
        progress: 100,
        uploading: false
      } : upload));
  ...
  return <div className="space-y-4">
      ...
      webkitdirectory: ""
    } as any} multiple onChange={handleFolderUpload} className="hidden" />
      <input ref={zipInputRef} type="file" accept=".zip" onChange={handleZipUpload} className="hidden" />

      {queuedFolders.length > 0 && <Card className="p-4">
          ...
          <div className="space-y-2">
            {queuedFolders.map(q => <div key={q.id} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
      ...
      {uploadingFiles.length > 0 && <Card className="p-4">
          <h4 className="font-semibold mb-3">Uploading Files</h4>
          <div className="space-y-3">
            {uploadingFiles.map((upload, index) => <div key={index} className="flex items-center space-x-3">
                {upload.isFolder ? <Folder className="h-5 w-5 text-blue-500" /> : <FileText className="h-5 w-5 text-gray-500" />}
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex flex-col">
                      ...
                      {upload.isFolder && upload.folderName && <span className="text-xs text-gray-500">
                          Folder: {upload.folderName}
                        </span>}
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => removeUpload(upload.file)} disabled={upload.uploading}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
      ...
      <NewFolderModal isOpen={showNewFolderModal} onClose={() => setShowNewFolderModal(false)} onCreateFolder={handleCreateFolder} />
    </div>;
}