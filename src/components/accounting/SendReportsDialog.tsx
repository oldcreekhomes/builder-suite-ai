...
export function SendReportsDialog({ projectId, open, onOpenChange }: SendReportsDialogProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // NOTE: This block must mirror BalanceSheetContent.tsx exactly so the
      // emailed PDF matches the on-screen Balance Sheet.
      if (reports.balanceSheet) {
        console.log('📊 Generating Balance Sheet PDF for project:', projectId);

        const [{ data: accounts, error: accountsError }, { data: exclusions }] = await Promise.all([
          supabase
            .from('accounts')
        ...
        const excludedAccountIds = new Set((exclusions || []).map((e: any) => e.account_id));

        // Page through ALL journal lines (matches on-screen fetchAllRows behavior)
