...
function StepItem({ step, navigate }: { step: any; navigate: (path: string) => void }) {
  return (
    <li
    ...
    >
      ...
      {!step.completed && step.link && (
        <Button
          ...
          className="ml-2 h-7 px-3 text-xs shrink-0"
        >
      ...
      <CardContent className="pt-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <ul className="space-y-2">
            {leftSteps.map((step) => (
              <StepItem key={step.key} step={step} navigate={navigate} />
            ))}
          </ul>
          <ul className="space-y-2">
            {rightSteps.map((step) => (
