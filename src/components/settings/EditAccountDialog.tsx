...
export function EditAccountDialog({ account, open, onOpenChange }: EditAccountDialogProps) {
  ...
  useEffect(() => {
    if (account) {
      setFormData({
        ...
        subtype: ((account as any).subtype as AccountSubtype) || "",
  ...
  const handleParentChange = (value: string) => {
    ...
    if (newParentId) {
      ...
      if (parent) setFormData((prev) => ({ ...prev, parent_id: newParentId, type: parent.type as AccountType }));
    }
  };

  const handleTypeChange = (value: string) => {
    setFormData((prev) => {
      const newState = { ...prev, type: value as AccountType, subtype: "" as AccountSubtype | "" };
      if (prev.parent_id) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.code || !formData.name) {
      toast({ title: "Validation Error", description: "Please enter both account code and name", variant: "destructive" });
      return;
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="code" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="e.g., 1000" required />
          </div>

          <div className="space-y-2">
            <Label htmlFor="name">Account Name</Label>
            <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Cash in Bank" required />
          </div>
          ...
          {subtypeOptions.length > 0 && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setFormData({ ...formData, subtype: v === "none" ? "" : (v as AccountSubtype) })}
              >
          ...
          <div className="space-y-2">
            ...
            <Input id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Account description" />
          </div>
