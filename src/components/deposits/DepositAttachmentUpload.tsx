...
interface DepositAttachmentUploadProps {
  attachments: DepositAttachment[];
  onAttachmentsChange: (attachments: DepositAttachment[]) => void;
  depositId?: string;
  disabled?: boolean;
}

export function DepositAttachmentUpload({ 
  attachments, 
  onAttachmentsChange, 
  depositId,
  disabled = false 
  ...
  const handleFileSelect = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    const newAttachments: DepositAttachment[] = [];
    ...
    for (const file of files) {
      ...
      // If we have a depositId, upload to storage immediately
      if (depositId) {
        try {
          ...
          const filePath = `deposit-attachments/${depositId}/${fileName}`;
        ...
        // If no depositId yet, just store the file temporarily
        const tempAttachment: DepositAttachment = {
          file_name: file.name,
    ...
    // Reset the input
    event.target.value = '';
  }, [attachments, onAttachmentsChange, depositId, disabled]);
  ...
  const handleDownloadAttachment = async (attachment: DepositAttachment) => {
    if (!attachment.id || !depositId) return;
