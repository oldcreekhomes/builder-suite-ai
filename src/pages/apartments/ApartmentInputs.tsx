    ...
    <div className="flex min-h-screen w-full">
      ...
      <div className="flex-1 flex flex-col">
        <header className="flex items-center h-12 border-b px-4">
          <SidebarTrigger />
          <h1 className="text-lg font-semibold ml-2">Apartment Inputs</h1>
        </header>
        <main className="flex-1 p-6 space-y-6 overflow-auto">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Field label="Taxes (Annual)" value={inputs.taxes} onChange={(v) => handleChange("taxes", v)} prefix="$" />
                <Field label="Insurance (Annual)" value={inputs.insurance} onChange={(v) => handleChange("insurance", v)} prefix="$" />
                <Field label="Utilities (Annual)" value={inputs.utilities} onChange={(v) => handleChange("utilities", v)} prefix="$" />
                ...
                <Field label="Payroll (Annual)" value={inputs.payroll} onChange={(v) => handleChange("payroll", v)} prefix="$" />
                <Field label="General & Admin" value={inputs.generalAdmin} onChange={(v) => handleChange("generalAdmin", v)} prefix="$" />
                <Field label="Marketing" value={inputs.marketing} onChange={(v) => handleChange("marketing", v)} prefix="$" />
                <Field label="Reserves per Unit ($/yr)" value={inputs.reservesPerUnit} onChange={(v) => handleChange("reservesPerUnit", v)} prefix="$" />
              </div>
            </CardContent>
          </Card>
          ...
          {/* Loan Inputs */}
          <Card>
            <CardHeader><CardTitle className="text-sm font-medium">Loan Inputs</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
