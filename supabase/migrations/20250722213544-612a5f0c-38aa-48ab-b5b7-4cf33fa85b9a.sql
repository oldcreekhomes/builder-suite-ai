...
-- Ensure parent_id column is properly set as text and update database functions
-- First, make sure the parent_id column is text (should already be from previous migration)
ALTER TABLE public.project_schedule_tasks 
ALTER COLUMN parent_id TYPE text USING parent_id::text;

-- Update the create_project_task function to handle text parent_id
...
-- Update the update_project_task function to handle text parent_id
...
-- Update the get_project_tasks function to return text parent_id
