...
-- Drop the old 8-parameter version that doesn't include review_notes
DROP FUNCTION IF EXISTS public.approve_pending_bill(uuid, uuid, uuid, date, date, text, text, text);

-- Replace the 9-parameter version to include purchase_order_id
...
BEGIN
  ...
  FROM pending_bill_lines WHERE pending_upload_id = pending_upload_id_param;
  ...
  LOOP
    INSERT INTO bill_lines (
      ...
      line_record.amount, line_record.memo,
      line_record.purchase_order_id
    );
  ...
  INSERT INTO bill_attachments (
    ...
    pending_upload_record.file_size, COALESCE(pending_upload_record.content_type, 'application/pdf'),
...
-- Backfill existing approved bills: copy purchase_order_id from pending_bill_lines
UPDATE bill_lines
SET purchase_order_id = pbl.purchase_order_id
FROM pending_bill_lines pbl
JOIN pending_bill_uploads pbu ON pbu.id = pbl.pending_upload_id
WHERE pbu.status = 'approved'
  AND bill_lines.line_number = pbl.line_number
  AND bill_lines.purchase_order_id IS NULL
  AND pbl.purchase_order_id IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM bills b
    WHERE b.id = bill_lines.bill_id
      AND b.owner_id = pbu.owner_id
  );
