-- Update approve_pending_bill to migrate pending bill_attachments to the newly approved bill
...
BEGIN
  ...
  LOOP
    INSERT INTO bill_lines (
      ...
      line_record.amount, COALESCE(line_record.memo, line_record.description),
  ...
  -- Check if there are bill_attachments linked via pending_upload_id
  SELECT EXISTS(
  ...
  IF has_pending_attachments THEN
    -- Migrate all pending attachments to the new bill
  ...
  ELSIF pending_upload_record.file_name IS NOT NULL AND pending_upload_record.file_name != '' THEN
    -- Legacy fallback: copy the single file from pending_bill_uploads
