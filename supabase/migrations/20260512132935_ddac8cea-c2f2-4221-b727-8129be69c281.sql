...
RETURNS TABLE(
  ...
  files_count bigint, budgets_count bigint, schedule_count bigint,
  photos_count bigint, chat_count bigint,
...
BEGIN
  ...
  -- Dynamic discovery: any public table with created_by + created_at
  FOR rec IN
    SELECT table_name,
  ...
  LOOP
    ...
    EXCEPTION WHEN others THEN
      -- Skip tables we can't read (permission etc.) without aborting
      NULL;
    END;
  END LOOP;
