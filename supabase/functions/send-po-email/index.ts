...
const generateFileDownloadLinks = (files: any[]) => {
  ...
  const fileLinks = files.map((file, index) => {
    ...
    return `<div style="line-height: 20px; margin: 2px 0; display: flex; align-items: center;">
      ...
      <span style="background-color: #10B981; color: #ffffff; font-size: 8px; font-weight: 600; padding: 1px 3px; border-radius: 2px; margin-left: 6px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; vertical-align: middle;">
        APPROVED
      </span>
    </div>`;
  }).join('');
